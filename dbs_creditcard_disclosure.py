...
# 星展 PDF 的中文常因字型編碼而無法可靠解碼，因此以法定揭露固定序號做主抽取。
TARGET_ITEM_MAP = {
    1: "circulating_cards",
    2: "valid_cards",
...
def _urlopen_with_ssl_fallback(req: Request, timeout: int = 30):
    try:
...
def request_text(url: str) -> str:
    ...
    with _urlopen_with_ssl_fallback(req, timeout=30) as resp:
        raw = resp.read()
...
def find_creditcard_pdf_url(entry_html: str, steps: list[str]) -> str:
    ...
    for href, inner_html in links:
        ...
        if PDF_LINK_TEXT in label:
            pdf_url = urljoin(ENTRY, html_lib.unescape(href).strip())
            steps.append(f"STEP 3: 入口頁找到『{PDF_LINK_TEXT}』PDF 連結：{pdf_url}")
            return pdf_url
    raise ValueError(f"入口頁找不到『{PDF_LINK_TEXT}』連結。")
...
def extract_pdf_text(pdf_bytes: bytes, steps: list[str]) -> str:
...
def extract_numbered_blocks(text: str) -> dict[int, str]:
    compact = text.replace("\r", "")
    blocks = re.findall(r"(?ms)(\d{1,2})\.\s*(.*?)(?=(?:\n\s*\d{1,2}\.)|\Z)", compact)
    result: dict[int, str] = {}
    for no_text, block in blocks:
        try:
            no = int(no_text)
        except ValueError:
            continue
        result[no] = re.sub(r"\s+", " ", block).strip()
    return result


def extract_metrics(text: str, steps: list[str]) -> dict:
    metrics: dict[str, str] = {}
    blocks = extract_numbered_blocks(text)

    for item_no, metric_key in TARGET_ITEM_MAP.items():
        block = blocks.get(item_no, "")
        if not block:
            steps.append(f"STEP 7: 找不到序號 {item_no} 對應區塊。")
            continue

        normalized = normalize_value(block)
        if normalized["ok"]:
            metrics[metric_key] = normalized["normalized"]
            steps.append(f"STEP 7: 依序號 {item_no} 擷取 {metric_key} = {normalized['normalized']}")
        else:
            steps.append(f"STEP 7: 序號 {item_no} 區塊存在，但數值清洗失敗：{block}")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="DBS disclosure extractor (standalone)")
...
def main() -> int:
    ...
    try:
        ...
        text = extract_pdf_text(pdf_bytes, steps)

        data_month, base_date_raw = extract_data_month(text, steps, args.month)
        metrics = extract_metrics(text, steps)
        ...
        if missing:
            result = build_result(
                ...
                base_date_raw,
                metrics,
                "部分欄位缺漏，需人工確認。星展 PDF 中文字型編碼可能導致部分文字無法正常解碼，本版以固定序號擷取主要指標。",
            ...
            result = build_result(
                ...
                base_date_raw,
                metrics,
                "星展 PDF 中文字型編碼可能無法正常解碼，因此本版依固定揭露序號抽取數值。",
                [],
                steps,
            )
