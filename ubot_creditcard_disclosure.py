...
def normalize_value(value_text: str) -> dict:
    ...
    Parse a value cell text like:
      '2,745,537', '14,138,753', '6,461,274', '0.19%', '—'
    Return:
...
# 聯邦銀行這支腳本由 SPA 入口頁 + 實際內容 HTML 抽取信用卡金融資訊。
...
# SPA entry page; used to mimic normal navigation flow.
ENTRY = "https://card.ubot.com.tw/ubcard/CreditCard_info"
# Actual HTML content loaded by the SPA page.
CONTENT_URL = "https://card.ubot.com.tw/ecard_source/html/ubcard/CreditCard_info.htm"
...
def fetch_entry_page(steps: list[str]) -> None:
    steps.append(f"STEP 1: 從固定入口抓取頁面：{ENTRY}")
    request_text(ENTRY)
    steps.append("STEP 2: 已進入聯邦信用卡金融資訊入口頁，準備抓取實際內容頁。")


def fetch_content_page(steps: list[str]) -> str:
    html = request_text(CONTENT_URL)
    steps.append(f"STEP 3: 已取得聯邦信用卡金融資訊內容頁 HTML：{CONTENT_URL}")
    return html
...
def extract_data_month(html: str, steps: list[str], fallback_month: str = "") -> tuple[str, str]:
    ...
    Returns (data_month, base_date_raw)
      ...
      - base_date_raw keeps original month text if found
    """
    base_date_raw = ""
    data_month = ""

    patterns = [
        r"<th[^>]*>\s*(\d{3,4}年\d{1,2}月)\s*</th>",
        r"<th[^>]*>\s*(\d{3,4}[\/\-.]\d{1,2})\s*</th>",
    ...
    for pattern in patterns:
        ...
        if m:
            ...
            if data_month:
                base_date_raw = raw
                steps.append(f"STEP 4: 從表頭找到資料月份：{raw} → {data_month}")
...
def build_result(
    ...
    return {
        ...
        "unit_note": "本腳本保留聯邦銀行來源 HTML 單位；金額欄位為仟元，卡數欄位為張。若需統一為百萬元，請由主控腳本集中轉換。",
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Union Bank credit card disclosure extractor (standalone)")
...
def main() -> int:
    ...
    try:
        fetch_entry_page(steps)
        html = fetch_content_page(steps)
